...
K_nums = np.array([0, 1,2, 5, 10, 25, 50])
...
eps_nums = np.concatenate([np.linspace(0.05,0.25,20), np.linspace(0.3,1.2,20), np.linspace(1.25,2,15)])
...
for i in [0, 18, 22,40]:
    ...
    dif = (gnval - dftemp.sort_values(["Epsilon", "K"])[1+
           i*len(K_nums):(i+1)*len(K_nums)-1]["Opt_val"])
    ax31.plot(K_nums[1:-1], dif, label=r"$\epsilon = {}$".format(
...
for i in [0,18, 22,40]:
    ax4.plot(K_nums[1:],
             dftemp.sort_values(["Epsilon", "K"])[i*len(K_nums):(i+1)*len(K_nums)]["solvetime"][1:],
             linestyle='-',
...
plt.savefig(foldername + "facilitybot.pdf")
