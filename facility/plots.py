...
for K_count in [0, 1, 4, 5, 7]:
    ax1.plot(eps_nums[::3], dftemp.sort_values(
        ["K", "Epsilon"])[K_count*len(eps_nums):(K_count+1) *
                          len(
            eps_nums)]["Opt_val"][::3],
        linestyle='-', marker=styles[j], label="$K = {}$".format(
...
#             dftemp.sort_values(["Epsilon", "K"])
# [i*len(K_nums):(i+1)*len(K_nums)]["solvetime"])[0],
