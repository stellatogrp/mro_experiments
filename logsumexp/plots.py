...
dftemp = pd.read_csv(foldername + 'df4_retry.csv')
...
for i in np.arange(0, len(eps_nums), 5):
    ...
    plt.plot(K_nums[:-1], dif[:-1], label="$\epsilon = {}$".format(
        np.round(eps_nums[i], 5)), linestyle='-', marker=styles[j], color=colors[j])
    plt.plot(K_nums[:-1], (dftemp.sort_values(["Epsilon", "K"])[i*len(K_nums):(i+1)*len(
        K_nums)]["bound"])[:-1], linestyle='--', color=colors[j], label="Upper bound")
    j += 1
