...
def createproblem(N, m,T,F,h,w):
    ...
    for k in range(N):
        for j in range(m):
            constraints += [cp.constraints.power.PowCone3D(-y[k,(j*T):((j+1)*T)],cp.hstack([F[j,t]*x[j] for t in range(1,T+1)]),cone[k,(j*T):((j+1)*T)],cp.hstack([t/(t+1) for t in range(1,T+1)]))]
    constraints += [cone <= 0]

    for k in range(N):
        constraints += [cp.sum([cp.sum([-y[k,(j*(T))+t-1] + cone[k,(j*(T))+t-1]*(t**(1/(t+1)) + t**(-t/(t+1))) for t in range(1,T+1)]) - F[j,0]*x[j] for j in range(m)]) - z[k]@dat[k] + cp.quad_over_lin(z[k]+ C_r.T@gam[k], 4*lam) + gam[k]@(d_r - C_r@dat[k]) <= s[k]]
        for j in range(m):
...
def createproblem_max(N, m,T,F,h,w):
    ...
    for k in range(N):
        constraints += [cp.sum([cp.sum([-y[k,(j*(T))+t-1] + cone[k,(j*(T))+t-1]*(t**(1/(t)) + t**(-t/(t+1))) for t in range(1,T+1)]) - F[j,0]*x[j] for j in range(m)]) - z[k]@dat[k] + cp.quad_over_lin(z[k], 4*lam) <= s[k]]
        for j in range(m):
            constraints += [cp.sum(y[k,(j*(T)):(j+1)*(T)]) == z[k,j]]
    constraints += [h@x <= theta]
    constraints += [lam >= 0]
