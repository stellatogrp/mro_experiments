...
def createproblem(N, m,T,F,h,w):
    ...
    lam = cp.Variable()
    z = cp.Variable((N, m))
    y = cp.Variable((N,(T+1)*m))
    obj = cp.Variable()
    cone = cp.Variable((N,(T+1)*m))
    ...
    for k in range(N):
        for j in range(m):
            constraints += [cone[k,(j*(T+1))] == -F[j,0]*x[j]]
            constraints += [cp.constraints.power.PowCone3D(-y[k,(j*(T+1) + 1):((j+1)*(T+1))],cp.hstack([F[j,t]*x[j] for t in range(1,T+1)]),cone[k,(j*(T+1) + 1):((j+1)*(T+1))],cp.hstack([t/(t+1) for t in range(1,T+1)]))]
    constraints += [cone <= 0]

    for k in range(N):
        constraints += [cp.sum([cp.sum([-y[k,(j*(T+1))+t] + cone[k,(j*(T+1))+t]*(t**(1/(t+1)) + t**(-t/(t+1))) for t in range(T+1)]) for j in range(m)]) - z[k]@dat[k] + cp.quad_over_lin(z[k]+ C_r.T@gam[k], 4*lam) + gam[k]@(d_r - C_r@dat[k]) <= s[k]]
        for j in range(m):
            constraints += [cp.sum(y[k,(j*(T+1)):(j+1)*(T+1)]) == z[k,j]]
    constraints += [h@x <= 12]
    constraints += [lam >= 0]
...
def createproblem_max(N, m,T,F,h,w):
    ...
    for k in range(N):
        constraints += [cp.sum([cp.sum([-y[k,(j*(T+1))+t] + cone[k,(j*(T+1))+t]*(t**(1/(t+1)) + t**(-t/(t+1))) for t in range(T+1)]) for j in range(m)]) - z[k]@dat[k] + cp.quad_over_lin(z[k], 4*lam) <= s[k]]
        for j in range(m):
            constraints += [cp.sum(y[k,(j*(T+1)):(j+1)*(T+1)]) == z[k,j]]
    constraints += [h@x <= 12]
    constraints += [lam >= 0]
...
def capital_experiment(R,r, m, N_tot, K_nums, eps_nums):
    ...
    '''
    df = pd.DataFrame(columns = ["R","K","Epsilon","Opt_val","Eval_val","satisfy","solvetime","bound","bound2"])
    xsols = np.zeros((len(K_nums),len(eps_nums),m, R))
    dat_val = (np.vstack([np.random.uniform(0.005*i,0.02*(i+1),int(N_tot/2)) for i in range(m)])).T
    d2 = (np.vstack([np.random.uniform(0.01*i,0.025*(i+1),int(N_tot/2)) for i in range(m)])).T
    dat_val = np.vstack([d,d2])
    d = (np.vstack([np.random.uniform(0.005*i,0.02*(i+1),int(N_tot/2)) for i in range(m)])).T
    d2 = (np.vstack([np.random.uniform(0.01*i,0.025*(i+1),int(N_tot/2)) for i in range(m)])).T
    ...
    for Kcount, K in enumerate(K_nums):
        ...
        for epscount, epsval in enumerate(eps_nums):
            if (K == N_tot):
                problem, x, dat, eps = createproblem_max(K, m,T,F,h,weights)
                eps.value = epsval**2
                dat.value = kmeans.cluster_centers_
                problem.solve()
                evalvalue = -np.mean([np.sum([np.sum([F[j,t]*x[j].value/(1+dat_eval[i,j])**t for t in range(T+1)]) for j in range(m)]) for i in range(N_tot)])
                L = np.linalg.norm([np.sum([(t**2 + t) * F[j,t]*x[j].value for t in range(T+1)]) for j in range(m)])
                L2 = np.max([np.sum([(t**2 + t) * F[j,t]*x[j].value for t in range(T+1)]) for j in range(m)])
                newrow = pd.Series(
                    {"R":r,
                      "K": 9999,
                      "Epsilon": epsval,
...
if __name__ == '__main__':
    ...
    results = Parallel(n_jobs=njobs)(delayed(capital_experiment)(R,r, m, N_tot, K_nums, eps_nums) for r in range(R))
    ...
    dftemp = dftemp/R
    dftemp.to_csv(foldername + '/df6.csv')

    all = pd.concat([results[r][1] for r in range(R)])
    all.to_csv(foldername + '/df_all6.csv')