...
ax1.plot(np.array(eps_nums)[::5],
         ...
         [K_count*len(eps_nums):(K_count+1)*len(
             eps_nums)]["Opt_val"][::5],
         linestyle='-', color=colors[j], marker=styles[j],
         label="$K = 100^*$", alpha=0.7)
ax1.plot(np.array(eps_nums)[::5],
         dftemp.sort_values(["K", "Epsilon"])[K_count*len(eps_nums):
                                              (K_count+1)*len(
             eps_nums)]["Eval_val"][::5], color=colors[j],
         linestyle=':', label="Expectation, $K = {}$".format(K_nums[K_count]))
...
j = 0
for K_count in [1, 3, 4, 7]:
    ax21.plot(1 - dftemp.sort_values(["K", "Epsilon"])
              [K_count*len(eps_nums):(K_count+1)*len(eps_nums)]
              ["satisfy"][0:-1:1], dftemp.sort_values(["K", "Epsilon"])[
              K_count*len(eps_nums):(K_count+1)*len(eps_nums)]
              ["Opt_val"][0:-1:1], linestyle='-',
              label="$K = {}$".format(K_nums[K_count]),
              marker=styles[j], alpha=0.7)
    j += 1
K_count = 0
ax21.plot(1 - dftemp.sort_values(
    ["K", "Epsilon"])[K_count*len(eps_nums):
                      (K_count+1)*len(eps_nums)]["satisfy"][0:-1:1],
    dftemp.sort_values(
    ["K", "Epsilon"])[K_count*len(eps_nums):
                      (K_count+1)*len(eps_nums)]["Opt_val"][0:-1:1],
    linestyle='-',
...
for i in [2, 10, 20, 30]:
    ...
    ax31.plot(K_nums[1:-1], dif[1:-1], label=r"$\epsilon = {}$".format(
        np.round(eps_nums[i], 5)),
        linestyle='-', marker=styles[j], color=colors[j])
    ax31.plot(K_nums[1:-1],
              (dftemp.sort_values(["Epsilon", "K"])[i*len(K_nums):(i+1) *
                                                    len(K_nums)]["bound"][1:-1]),
              linestyle='--', color=colors[j], label="Upper bound")
    j += 1
...
j = 0
for i in [2, 10, 20, 30]:
    ax4.plot(K_nums[1:], dftemp.sort_values(["Epsilon", "K"])
             [i*len(K_nums):(i+1)*len(K_nums)]["solvetime"][1:],
             linestyle="-", marker=styles[j], color=colors[j],
             label=r"$\epsilon = {}$".format(np.round(eps_nums[i], 5)))
    j += 1
